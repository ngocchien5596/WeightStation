...
{
    private ClayWeighingViewModel? _viewModel;
    ...
    private async void SessionSearchBox_KeyDown(object sender, KeyEventArgs e)
    {
        if (e.Key == Key.Enter && DataContext is ClayWeighingViewModel vm)
        {
    ...
    private void OnDataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
    {
        ...
        _viewModel = e.NewValue as ClayWeighingViewModel;
        if (_viewModel != null)
        {
            _viewModel.PropertyChanged += OnViewModelPropertyChanged;
        }
    ...
    {
        if (e.PropertyName != nameof(ClayWeighingViewModel.ClearTripSelectionRequest))
        {
            return;
        }
    ...
    private void OnTripsGridPreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        ...
        if (FindAncestor<DataGridRow>(e.OriginalSource as DependencyObject) is not { Item: ClayVehicleTripListItem trip } row)
        {
            return;
        }
    ...
    private void OnReturnedBrokenTripCheckBoxPreviewMouseDown(object sender, MouseButtonEventArgs e)
    {
        ...
        if (sender is CheckBox { DataContext: ClayVehicleTripListItem trip }
            && _viewModel is ClayWeighingViewModel vm)
        {
            _ = vm.ToggleReturnedBrokenTripCommand.ExecuteAsync(trip);
        }
    }
    ...
    private void LogTripGridSelectionState(string source, bool isResettingSelection = false)
    {
        _viewModel?.LogTripGridSelectionState(
            source,
            TripsDataGrid.SelectedItem as ClayVehicleTripListItem,
            TripsDataGrid.SelectedIndex,
            TripsDataGrid.CurrentItem as ClayVehicleTripListItem,
