...
def enum_windows_callback(hwnd, lParam):
    if user32.IsWindowVisible(hwnd):
        ...
        if length > 0:
            ...
            if any(k in title_lower for k in ["trạm cân", "đăng nhập", "stationapp", "weightstation"]):
...
def main():
    ...
    # ASCII clean title for console print
    clean_title = title.encode('ascii', 'ignore').decode('ascii')
    print(f"Found window: {clean_title} (HWND: {hwnd})")
    ...
    if user32.GetWindowRect(hwnd, ctypes.byref(rect)):
        ...
        img.save(os.path.join(output_dir, "login_window.png"), "PNG")
        print("Login window captured successfully!")
    else:
