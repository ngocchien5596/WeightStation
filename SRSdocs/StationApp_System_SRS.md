...
| 1.6 | 11/06/2026 | Codex | Cập nhật cấu hình giờ sao lưu DB local theo định dạng 24h, sửa công thức quy đổi bao, bổ sung hành vi dropdown tìm kiếm báo cáo, hiển thị mã chứng từ đồng bộ, và mở rộng hàm ERP lấy SL thực xuất trả thêm thời gian cân lần 1/lần 2. |
...
| **Cắt lệnh tạm xuất khẩu** | Cắt lệnh nội bộ tại trạm cân dùng cho luồng cân xuất khẩu khi xe vào cân trước nhưng ERP chưa truyền cắt lệnh thật, hoặc khi ERP RA cắt lệnh xuất khẩu đã có chuyến xe. Mã hiển thị theo format `CL-TAM-####`. |
...
| `IsTemporaryExport` | `bit` | NOT NULL, DEFAULT 0 | Đánh dấu cắt lệnh tạm xuất khẩu do trạm cân tạo hoặc do hệ thống tạo khi ERP RA cắt lệnh đã có chuyến xe. |
| `TemporaryExportDisplayCode` | `nvarchar(100)` | NULL | Mã hiển thị cắt lệnh tạm, format `CL-TAM-####`, dùng trên UI thay cho mã ERP thật khi chưa có cắt lệnh thật. |
| `TemporaryExportCreatedReason` | `nvarchar(50)` | NULL | Lý do tạo cắt lệnh tạm, ví dụ `MANUAL_PRELOAD` hoặc `ERP_REISSUE_HOLDING`. |
| `TemporaryExportSourceErpCutOrderId` | `nvarchar(100)` | NULL | Mã cắt lệnh ERP nguồn khi cắt lệnh tạm được tạo để giữ chuyến sau thao tác RA/CO. |
| `MappedRealCutOrderId` | `uniqueidentifier` | FK, NULL | Trên cắt lệnh tạm, lưu ID cắt lệnh thật sau khi map thành công. |
| `MappedTemporaryCutOrderId` | `uniqueidentifier` | FK, NULL | Trên cắt lệnh thật, lưu ID cắt lệnh tạm nguồn đã được map. |
| `MappedAt` | `datetime2` | NULL | Thời điểm map cắt lệnh tạm sang cắt lệnh thật. |
| `MappedBy` | `nvarchar(100)` | NULL | Tài khoản thực hiện map cắt lệnh tạm sang cắt lệnh thật. |
...
Các khóa cấu hình nghiệp vụ chính:
- `station_code`: mã trạm cân.
- `ticket_prefix`: tiền tố phiếu cân.
- `delivery_prefix`: tiền tố phiếu giao nhận.
- `tolerance_kg_per_bag`: dung sai cho 1 bao.
- `sync_interval_seconds`: chu kỳ đồng bộ outbox.
- `registration_inbound_poll_seconds`: chu kỳ thăm dò đăng ký ERP.
- `central_api_url`: URL Central API.
- `central_api_key`: API key xác thực Central API.
- `local_database_backup_directory`: thư mục lưu file backup `.bak`.
- `local_database_backup_time`: giờ tự động backup DB local theo định dạng 24 giờ `HH:mm`, mặc định `01:00`.
...
- **ERP SQL Contract - Lấy số lượng thực xuất**: ERP gọi `dbo.sp_GetCutOrderNetWeight @ErpCutOrderId = ?` hoặc truy vấn `dbo.fn_GetCutOrderNetWeight(@ErpCutOrderId)` trên DB local để lấy kết quả cân của một cắt lệnh. Contract trả về các cột:
  - `NetWeightTon decimal(18,3)`: số lượng thực xuất/thực nhập theo đơn vị tấn.
  - `Weight1Time datetime2(7)`: thời gian lưu cân lần 1 sớm nhất của các lượt cân liên quan.
  - `Weight2Time datetime2(7)`: thời gian lưu cân lần 2 muộn nhất của các lượt cân liên quan.
  - Contract hiện tại không trả `PickupTime` hoặc `PickupTimeText`; ERP cần dùng trực tiếp `Weight1Time` và `Weight2Time` để ghi nhận thời gian cân.
  - Với cắt lệnh xuất khẩu (`IsExportScale = 1`), chỉ trả kết quả hợp lệ sau khi cắt lệnh đã chốt tổng (`ExportFinalizedWeight IS NOT NULL`). Nếu chưa chốt tổng, procedure phải báo lỗi để tránh ERP lấy số lượng tạm.
...
| UI-INC-TXT-SEARCHBS | `TextBox` | `SearchVehiclePlate` | Trường biển số xe tìm kiếm dạng text thường, không hiển thị danh sách gợi ý trên thanh header. Nhấn Enter để thực hiện. |
...
| UI-INC-FORM-TXT-CUSTCODE | `AutocompleteTextBox` | `FormCustomerCodeInput` | Nhập mã khách hàng. Hỗ trợ gợi ý theo ký tự nhập tương tự biển số xe; khi chọn mã khách hàng, hệ thống tự động điền tên khách hàng tương ứng. |
...
- **Nút "Lưu thay đổi" (`SaveDetailCommand`)**: 
  ...
  - Tiến hành kiểm tra tính hợp lệ của dữ liệu (Validate): với luồng xuất hàng, biển số xe, tên tài xế, tên khách hàng, mã/tên sản phẩm, khối lượng kế hoạch không được để trống hoặc nhỏ hơn hoặc bằng 0; nếu sản phẩm là hàng bao, trường số lượng bao phải được nhập. Với luồng nhập hàng (`INBOUND`), tên tài xế và khối lượng kế hoạch không bắt buộc; hệ thống chỉ cần ghi nhận cân lần 1, cân lần 2 và tính khối lượng hàng thực tế.
...
- **Nút "Cân xuất khẩu" (`TransitionToExportScaleCommand`)**:
  ...
  - Gọi `TransitionToExportScaleUseCase` để chuyển đổi trạng thái của cắt lệnh cha sang luồng cân xuất khẩu lũy kế cộng dồn.
  - Nếu hệ thống phát hiện có cắt lệnh tạm xuất khẩu đang hoạt động, hệ thống hiển thị modal map để người dùng chọn cắt lệnh tạm phù hợp với cắt lệnh thật. Danh sách gợi ý ưu tiên các cắt lệnh tạm cùng khách hàng, cùng sản phẩm hoặc có ghi chú liên quan.
  - Nếu người dùng xác nhận map, toàn bộ chuyến xe của cắt lệnh tạm được chuyển sang cắt lệnh thật trước khi điều hướng sang `ExportWeighingView`. Nếu người dùng bỏ qua map, hệ thống vẫn điều hướng sang `ExportWeighingView` và giữ nguyên luồng tạo chuyến trực tiếp trên cắt lệnh thật.
...
- **Gợi ý tự động (Autocomplete)**: Khi nhân viên gõ tối thiểu 1 ký tự đối với biển số xe/mooc/mã khách hàng/mã sản phẩm, hoặc 2 ký tự đối với tên tài xế/khách hàng/tên sản phẩm, hệ thống sẽ gọi `IAutocompleteService.SearchAsync` bất đồng bộ để tìm kiếm các bản ghi khớp trong danh mục cục bộ và hiển thị danh sách gợi ý xổ xuống ngay dưới ô nhập liệu. Riêng ô lọc biển số xe trên thanh header của màn Danh sách xe vào là ô tìm kiếm thường, không bật gợi ý.
- **Tự động điền thông tin (Auto-fill)**:
  ...
  - **Tự động đồng bộ tên khách hàng qua mã**: Khi nhân viên chọn mã khách hàng từ gợi ý hoặc gõ thủ công mã khách hàng vào ô `Mã KH` (`FormCustomerCodeInput`) rồi di chuyển tiêu điểm ra ngoài (Lost Focus), hệ thống sẽ chạy tiến trình ngầm `SyncCustomerByCodeAsync` để tự động kiểm tra mã trong danh mục khách hàng và điền tên khách hàng tương ứng lên Form.
...
| UI-WGH-BTN-SPLIT | `Button` | `ShowOverweightHandlingCommand` | Mở hộp thoại xử lý quá tải tải trọng thiết kế của xe (Tách tải). Nút này chỉ hiển thị khi phiên cân thực sự phát sinh trạng thái quá tải cần xử lý, tránh thao tác nhầm trong luồng cân thông thường. |
...
- **Ràng buộc kiểm tra (Validation)**:
  ...
  - Với sản phẩm hàng bao, số bao quy đổi thực tế sẽ được tính toán tự động bằng phần nguyên của `ActualAllocatedWeight / 50`; chỉ khi khối lượng vượt đủ bội số 50 kg mới tăng thêm 1 bao. Ví dụ `70.039 kg` tương ứng `1.400` bao, không làm tròn lên `1.401` bao.
...
- **Nút "TẠO CẮT LỆNH TẠM" (`CreateTemporaryCutOrderCommand`)**:
  - Cho phép nhân viên cân tạo một cắt lệnh tạm xuất khẩu ngay trên màn Cân xuất khẩu trong tình huống xe vào cân xác trước, nhưng ERP chưa chốt lô và chưa truyền cắt lệnh thật xuống trạm cân.
  - Trước khi tạo, hệ thống hiển thị hộp thoại xác nhận để tránh nhân viên bấm nhầm. Chỉ sau khi người dùng xác nhận, UseCase tạo cắt lệnh tạm mới được thực thi.
  - Hệ thống sinh mã hiển thị tuần tự theo format `CL-TAM-####`, đặt `IsExportScale = true`, `IsTemporaryExport = true`, `CutOrderStatus = REGISTERED`, `ProcessingStage = IN_YARD`.
  - Cắt lệnh tạm chỉ được dùng nội bộ tại trạm cân để tạo chuyến xe và ghi nhận cân lần 1/lần 2; không được coi là cắt lệnh thật để ERP lấy số lượng thực xuất.
...
- **Nút "CHỐT TỔNG" (`FinalizeCommand`)**:
  ...
  - Nếu dòng đang chọn là cắt lệnh tạm (`IsTemporaryExport = true`), nút **CHỐT TỔNG** phải bị vô hiệu hóa. Cắt lệnh tạm bắt buộc phải map sang cắt lệnh thật trước khi được phép chốt tổng và cung cấp số lượng thực xuất cho ERP.
...
##### 4. Cơ chế cắt lệnh tạm và map sang cắt lệnh thật (FR-EXPORT-002)
- **Tạo cắt lệnh tạm trước khi ERP truyền cắt lệnh thật**: Khi xe vào cân trước, nhân viên tạo cắt lệnh tạm `CL-TAM-####` trên màn Cân xuất khẩu, sau đó tạo các chuyến xe con và cân như luồng xuất khẩu hiện tại.
- **Nhận cắt lệnh thật từ ERP**: Sau khi ERP chốt lô và truyền cắt lệnh thật xuống, cắt lệnh thật xuất hiện ở màn Danh sách xe vào. Khi người dùng chọn cắt lệnh thật và bấm **Cân xuất khẩu**, hệ thống chuyển trạng thái cắt lệnh thật sang luồng xuất khẩu và kiểm tra các cắt lệnh tạm đang hoạt động.
- **Xác định cắt lệnh tạm đang hoạt động**: Cắt lệnh tạm được coi là đang hoạt động khi `IsTemporaryExport = true`, `IsExportScale = true`, chưa bị xóa/hủy, chưa chốt tổng, chưa map sang cắt lệnh thật khác, và còn ở trạng thái vận hành cho phép tạo/cập nhật chuyến xe.
- **Gợi ý cắt lệnh tạm phù hợp**: Modal map ưu tiên gợi ý theo khách hàng, sản phẩm và ghi chú. Không bắt buộc so khớp `TemporaryExportSourceErpCutOrderId` với `ErpCutOrderId` vì cắt lệnh tạm có thể được tạo trước khi biết mã cắt lệnh thật.
- **Map cắt lệnh tạm sang cắt lệnh thật**: Khi nhân viên xác nhận map, hệ thống chuyển toàn bộ `weighing_session_lines`, `delivery_tickets`, quan hệ phiên cân và lũy kế sản lượng từ cắt lệnh tạm sang cắt lệnh thật. Cắt lệnh thật trở thành nguồn dữ liệu chính để chốt tổng và để ERP gọi lấy số lượng thực xuất.
- **Bảo toàn luồng hiện tại**: Nếu không có cắt lệnh tạm phù hợp hoặc nhân viên bỏ qua thao tác map, cắt lệnh thật vẫn tiếp tục đi theo luồng cân xuất khẩu hiện tại, cho phép tạo chuyến xe trực tiếp trên cắt lệnh thật.

##### 5. Xử lý ERP RA/CO cắt lệnh xuất khẩu đã có chuyến (FR-REISSUE-002)
- Khi ERP thực hiện RA một cắt lệnh xuất khẩu đã có chuyến xe, hệ thống không được để các chuyến xe này chuyển sang màn Cân nội địa và không được mất quan hệ nghiệp vụ xuất khẩu.
- Stored procedure cấp lại cắt lệnh phải tạo hoặc tái sử dụng một cắt lệnh tạm xuất khẩu `CL-TAM-####`, sau đó gắn các chuyến xe, dòng phân bổ, phiếu cân và phiếu giao nhận đang liên quan vào cắt lệnh tạm này.
- Các chuyến xe sau RA vẫn hiển thị ở màn Cân xuất khẩu vì bản chất nghiệp vụ vẫn là xuất khẩu (`IsExportScale = true`). Màn Cân nội địa không được hiển thị dữ liệu của cắt lệnh hoặc chuyến xe xuất khẩu.
- Khi ERP CO lại và truyền cắt lệnh thật mới, người dùng map cắt lệnh thật với cắt lệnh tạm để khôi phục toàn bộ chuyến xe vào cắt lệnh thật, đảm bảo ERP lấy đúng số lượng thực xuất sau cùng.

##### 6. Cơ chế gợi ý tự động (Autocomplete) và Điền thông tin tự động (Auto-fill)
...
- **Quy tắc hiển thị khối lượng kế hoạch đối với chuyến xe xuất khẩu**: Với các dòng thuộc cắt lệnh xuất khẩu (`IsExportScale = true`), cột `KL KH` trên lưới Danh sách xe ra phải hiển thị bằng `KL Thực` của chuyến xe. Lý do là luồng xuất khẩu có thể cân nhiều chuyến trên một cắt lệnh lớn, khối lượng kế hoạch của cắt lệnh là tổng kế hoạch toàn lô, không đại diện cho từng chuyến xe con.
...
| **UI-CFG-SYS-SPLITSTEP**| `TextBox` | `OverweightSplitStepWeight` | Tỉ lệ tách tải tối đa (%) dùng trong thuật toán xử lý quá tải. |
...
| **UI-CFG-SYS-BKTIME**  | `TextBox` | `LocalDatabaseBackupTime` | Giờ tự động sao lưu DB local theo định dạng 24 giờ `HH:mm`. Ví dụ `01:00` là 1 giờ sáng, `13:30` là 1 giờ 30 chiều. Giá trị mặc định: `01:00`. |
...
- **Tác vụ Tự động Sao lưu CSDL cục bộ (FR-BACKUP-001)**: Tác vụ chạy ngầm `LocalDatabaseBackupWorker` chạy định kỳ mỗi ngày theo giờ cấu hình `LocalDatabaseBackupTime`. Worker đọc key `local_database_backup_time` từ `app_config`, chấp nhận định dạng `HH:mm` hoặc `HH:mm:ss` theo chuẩn 24 giờ. Nếu cấu hình trống hoặc sai định dạng, hệ thống dùng mặc định `01:00`. Đúng thời điểm hợp lệ tiếp theo, worker gọi câu lệnh backup cơ sở dữ liệu SQL Server lưu vào thư mục `LocalDatabaseBackupDirectory` dưới định dạng tệp `yyyyMMdd_{DatabaseName}.bak`.
...
- **Hiển thị mã chứng từ (`DocumentCode`)**: Với loại dữ liệu ĐKPT/CutOrder, màn Cấu hình đồng bộ phải ưu tiên hiển thị mã nghiệp vụ người dùng nhận biết được như `ErpCutOrderId` hoặc mã cắt lệnh hiển thị, không hiển thị GUID kỹ thuật của aggregate. Với các loại chứng từ khác, hệ thống hiển thị số phiếu nghiệp vụ tương ứng như PC/PGN hoặc mã phiên cân.
...
### 4.7.6.1 Lịch sử cân PM cũ (ExternalDatacanView.xaml)

Màn **Lịch sử cân (PM cũ)** cho phép tài khoản Admin tra cứu dữ liệu cân lịch sử từ hệ thống cân cũ qua database ngoài `XIM_TRANSPORT`, bảng `dbo.Datacan`. Màn này chỉ phục vụ đối chiếu/tra cứu, không ghi dữ liệu vào database cũ, không đồng bộ vào `StationAppLocal`, không tham gia tính KPI, báo cáo hoặc chứng từ vận hành hiện tại.

#### Quyền truy cập

- Chỉ người dùng có role `ADMIN` được nhìn thấy menu **Cấu hình hệ thống > Lịch sử cân (PM cũ)**.
- Người dùng không phải Admin không thấy menu và không được truy cập tab nội bộ tương ứng trong `SettingsView`.

#### Nguồn dữ liệu

- Connection string: `ConnectionStrings:ExternalDatacanConnection` trong `appsettings.json`.
- Database nguồn: `XIM_TRANSPORT`.
- Bảng nguồn: `dbo.Datacan`.
- Service đọc dữ liệu: `ExternalDatacanQueryService`.
- Interface: `IExternalDatacanQueryService`.
- DTO hiển thị: `ExternalDatacanRecordDto`.

#### Danh sách phần tử giao diện chính

| Mã phần tử | Loại Control | Tên / Binding | Mô tả chức năng |
| :--- | :--- | :--- | :--- |
| **UI-CFG-OLDPM-TXT-VEH** | `TextBox` | `VehiclePlateKeyword` | Nhập từ khóa tìm kiếm theo biển số xe cũ (`dbo.Datacan.Soxe`). |
| **UI-CFG-OLDPM-TXT-PRD** | `TextBox` | `ProductKeyword` | Nhập từ khóa tìm kiếm theo hàng hóa/sản phẩm (`dbo.Datacan.Hanghoa`). |
| **UI-CFG-OLDPM-TXT-CUS** | `TextBox` | `CustomerKeyword` | Nhập từ khóa tìm kiếm theo khách hàng (`dbo.Datacan.Khachhang`). |
| **UI-CFG-OLDPM-BTN-SRCH** | `Button` | `SearchCommand` | Tải lại trang đầu tiên theo bộ lọc hiện tại. |
| **UI-CFG-OLDPM-BTN-REFR** | `Button` | `RefreshCommand` | Làm mới dữ liệu theo bộ lọc hiện tại. |
| **UI-CFG-OLDPM-GRID** | `DataGrid` | `ItemsSource={Binding Records}` | Lưới dữ liệu read-only hiển thị lịch sử cân từ PM cũ. |
| **UI-CFG-OLDPM-BTN-PREV** | `Button` | `PreviousPageCommand` | Chuyển về trang trước. |
| **UI-CFG-OLDPM-BTN-NEXT** | `Button` | `NextPageCommand` | Chuyển sang trang sau. |
| **UI-CFG-OLDPM-TXT-PAGE** | `TextBlock` | `PageSummary` | Hiển thị số trang hiện tại và số dòng đang hiển thị. |
| **UI-CFG-OLDPM-TXT-ERR** | `TextBlock` | `ErrorMessage` | Hiển thị lỗi kết nối hoặc lỗi truy vấn database ngoài. |

#### Mapping cột dữ liệu

| Cột UI | Cột nguồn `dbo.Datacan` | Định dạng |
| :--- | :--- | :--- |
| Số phiếu | `Sophieu` | Text |
| Biển số xe | `Soxe` | Text, căn trái |
| Loại | `Nhomhang` | Text |
| Khách hàng | `Khachhang` | Text |
| Hàng hóa | `Hanghoa` | Text |
| Giờ cân lần 1 | `Ngayvao` | `dd/MM/yyyy HH:mm:ss` |
| Giờ cân lần 2 | `Ngayra` | `dd/MM/yyyy HH:mm:ss` |
| Cân lần 1 | `KLxe` | `N0` |
| Cân lần 2 | `KLTong` | `N0` |
| Trọng lượng hàng | `KLhang` | `N0` |
| Người cân | `Nvc` | Text |

#### Nghiệp vụ xử lý liên quan

- Khi mở màn lần đầu, `ExternalDatacanViewModel.LoadAsync()` gọi service để tải trang đầu tiên.
- Bộ lọc tìm kiếm dùng logic `AND`: nếu nhập nhiều điều kiện, bản ghi phải thỏa mãn tất cả điều kiện.
- Dữ liệu được sắp xếp theo `Ngayra DESC`; bản ghi chưa có `Ngayra` được đưa xuống sau.
- Mỗi lần tải chỉ lấy một trang dữ liệu, mặc định 100 dòng, để tránh treo UI nếu bảng `dbo.Datacan` lớn.
- `DataGrid` bật row/column virtualization và không cho sửa dữ liệu.
- Nếu connection string chưa cấu hình, sai thông tin đăng nhập, sai IP hoặc bảng nguồn không tồn tại, hệ thống hiển thị lỗi trên màn và không làm crash ứng dụng.

#### Yêu cầu bảo mật

- Không log connection string hoặc password.
- Không hiển thị password database ngoài trên UI.
- Khuyến nghị vận hành dùng SQL user read-only thay cho tài khoản `sa`, chỉ cấp quyền `SELECT` trên `dbo.Datacan`.
...
stateDiagram-v2
    ...
    [*] --> TEMP_EXPORT : Tạo cắt lệnh tạm xuất khẩu CL-TAM-####
    REGISTERED --> IN_SESSION : Được gắn vào một phiên cân đang hoạt động (WEIGHING)
    REGISTERED --> TEMP_EXPORT : ERP RA cắt lệnh xuất khẩu đã có chuyến
    TEMP_EXPORT --> REGISTERED : Map chuyến xe sang cắt lệnh thật sau ERP CO lại
...
| Tạo và map cắt lệnh tạm xuất khẩu | FR-EXPORT-002 | NFR-REL-002, NFR-PERF-003 | Đã kiểm chứng |
| Xử lý cấp lại đơn cắt lệnh của ERP | FR-REISSUE-001, FR-REISSUE-002 | NFR-REL-002 | Đã kiểm chứng |
| Hiển thị khối lượng chuyến xe xuất khẩu ở Danh sách xe ra | FR-OUT-002 | NFR-USA-001 | Đã kiểm chứng |
| Tự động sao lưu dữ liệu cục bộ | FR-BACKUP-001 | NFR-REL-001 | Đã kiểm chứng |
| Cấu hình giờ sao lưu dữ liệu cục bộ | FR-BACKUP-002 | NFR-USA-001, NFR-REL-001 | Đã kiểm chứng |
| ERP lấy số lượng thực xuất và thời gian cân | FR-ERP-001 | NFR-REL-002 | Đã kiểm chứng |
